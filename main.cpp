...
int interpret(bool shell, const std::string &filename) {
    ...
    if (!shell) {
        ...
        if (!file.is_open()) {
            std::cout << RED << "Błąd: Nie można otworzyć pliku " << filename << RST << std::endl;
            return 1;
        }
    }
    std::string multiline, line;
    bool continuation;
    if (shell) std::cout << "Wpisz 'wyjdz' aby wyjść" << std::endl;
    ...
    while (true) {
        ...
        try {
            do {
                ...
                if (shell && line == "wyjdz" && multiline.empty()) {
                    return 0;
                }
            ...
            std::cout << RED << "Błąd przepływu sterowania: Użycie " << e.what() << " poza pętlą" << RST << std::endl;
...
int main(int argc, char *argv[]) {
    if (argc > 2) {
        std::cout << "Użycie: <program> [<nazwa_pliku>]" << std::endl;
        return 1;
    }
    std::cout << std::fixed;
